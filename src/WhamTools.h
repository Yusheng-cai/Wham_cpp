...
Matrix<Real> Hessian(const Matrix<Real>& BUki, const std::vector<Real>& fk,
                     const std::vector<Real>& N);

// find the gradient vector of the UWham NLL equation
std::vector<Real> Gradient(const Matrix<Real>& BUki, const std::vector<Real>& fk,
                           const std::vector<Real>& N);

// find the gradient vector of the Bwham NLL equation
std::vector<Real> BGradient(const Matrix<Real>& BWil, const std::vector<Real>& Ml,
                            const std::vector<Real>& N, const std::vector<Real>& fk);

// find the lnWi in UWham
std::vector<Real> calculatelnWi(const Matrix<Real>& BUki, const std::vector<Real>& fk,
                                const std::vector<Real>& N);

// find the lnpl in Bwham
std::vector<Real> calculatelnpl(const Matrix<Real>& BWil, const std::vector<Real>& Ml,
                                const std::vector<Real>& N, const std::vector<Real>& fk);
...
Real CalculateDeltaFBarIterative(const std::vector<Real>& w_F, const std::vector<Real>& w_B,
                                 int maxiterations = 500, Real tolerance = 1e-7);

// Estimate free energy difference using BAR --> bisection method
Real CalculateDeltaFBarBisection(const std::vector<Real>& w_F, const std::vector<Real>& w_B,
                                 int maxiterations = 500);

// Calculates the Uwham NLL equation
Real Uwham_NLL_equation(const std::vector<Real>& f_k, const Matrix<Real>& BUji,
                        const std::vector<Real>& N);
}; // namespace WhamTools
