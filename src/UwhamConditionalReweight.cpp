...
UwhamConditionalReweight::UwhamConditionalReweight(const ReweightInput& input) : Reweight(input)
{
    // check if the Wham passed in is Uwham
    uwhamptr_ = dynamic_cast<Uwham*>(wham_);
    ASSERT((uwhamptr_ != nullptr),
           "The reweight procedure does not apply to the wham with name " << uwhamptr_->getName());
    ...
    // register average
    output_->registerOutputFunc("ConditionalAverage", [this](std::string name) -> void {
        this->printConditionalAverage(name);
    });
...
void UwhamConditionalReweight::calculate()
{
    ...
    ConditionalAverage_.resize(
        numBias_, std::vector<std::vector<Real>>(numbins, std::vector<Real>(dimension_, 0.0)));
...
#pragma omp parallel for
        ...
        for (int j = 0; j < ConditionalIndex_.size(); j++) {
            if (ConditionalIndex_[j].size() > 0) {
                // fill the weight and data given the dimension
