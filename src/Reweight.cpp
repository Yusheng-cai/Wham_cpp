...
Reweight::Reweight(const ReweightInput& input) : wham_(input.wham_), pack_(input.pack_)
{
    ...
    ASSERT((outputNames_.size() == outputfileNames_.size()),
           "input size does not match with output.");
    ...
    for (int i = 0; i < biasPacks.size(); i++) {
        ...
        Vectorbias_.push_back(
            Biasptr(BiasRegistry::Factory::instance().create(type, *biasPacks[i])));
    }
