...
HistogramOP::HistogramOP(const TSInput& input) : TSoperation(input)
{
    ...
    outputs_->registerOutputFunc("histogram",
                                 [this](std::string name) -> void { this->printHistogram(name); });
    outputs_->registerOutputFunc(
        "totalhistogram", [this](std::string name) -> void { this->printTotalHistogram(name); });
}
