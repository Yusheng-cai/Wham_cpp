...
namespace {
std::unique_ptr<UWhamCalculationStrategy> makeUwhamStrategy(const std::string& type,
...
Uwham::Uwham(const WhamInput& input) : Wham(input)
{
    ...
    registerOutput("normalization",
                   [this](std::string name) -> void { this->printNormalization(name); });
...
void Uwham::initializeBUki()
{
    if (verbose_) {
        ...
        std::cout << "The space it takes is around "
                  << Biases_.size() * xi_.size() * sizeof(Real) / 1e6 << " Mb" << std::endl;
    }
...
void Uwham::MakeInitialGuess(const Matrix<Real>& BUki, const std::vector<Real>& N,
                             std::vector<Real>& fk)
{
    // inital guess for fk
    fk.clear();
...
void Uwham::initializeStrat(Matrix<Real>& BUki, std::vector<Real>& N,
                            std::vector<stratptr>& strategies)
{
    ...
    for (auto s : stratPacks) {
        ...
        ASSERT((inserted.second), "Strategy name "
                                      << strategyName
                                      << " is already registered in this WHAM calculation.");
    }

    // read a vector of string that represents the order of optimization that we want to do ,
    // usually LBFGS --> adaptive
    ...
    for (auto s : strategyNames) {
        ...
        ASSERT((stratit->second != nullptr),
               "Strategy name " << s << " is requested more than once.");
