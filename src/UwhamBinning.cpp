...
void Uwham::bindata(std::vector<std::vector<Real>>& xi,
                    std::map<std::vector<int>, std::vector<int>>& map,
                    std::vector<std::vector<int>>& DataBinIndex)
{
...
#pragma omp for
        for (int i = 0; i < xi.size(); i++) {
            ...
            for (int j = 0; j < Bins_.size(); j++) {
                ...
                // If this data point is out of range for one of the bins, it is out of range, so we
                // break
...
void Uwham::calculateFreeEnergy(const std::vector<Real>& lnwji,
                                const std::map<std::vector<int>, std::vector<int>>& map,
...
void Uwham::ReduceFEDimension()
{
    ...
    for (int i = 0; i < Bins_.size(); i++) {
        ...
        for (auto it = MapBinIndexTolnwjiIndex_.begin(); it != MapBinIndexTolnwjiIndex_.end();
             it++) {
        ...
        for (auto it = MapDimBinNumTolnwjiIndex.begin(); it != MapDimBinNumTolnwjiIndex.end();
             it++) {
...
{
    ASSERT((num < Bins_.size()),
           "The dimension provided is larger than the total number of dimensions which is "
               << Bins_.size());
