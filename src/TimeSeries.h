...
public:
    ...
    bool is_constant_dimension(const std::vector<std::vector<Real>>& data, int dim,
                               float tol = 1e-6);
    ...
    // obtain a randomly sampled points from the data that equals to the number of independent
    // points
...
private:
    ...
    // Sometimes it is ideal to skip some data in the beginning because the simulation has not
    // reached eq. yet
