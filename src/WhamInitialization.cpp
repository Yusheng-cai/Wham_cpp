...
Wham::Wham(const WhamInput& input) : VectorTimeSeries_(input.VectorTimeSeries_), pack_(input.pack_)
{
    ASSERT((VectorTimeSeries_.size() != 0), "No timeseries data was passed in.");

    whamPack_ =
        const_cast<ParameterPack*>(pack_.findParamPack("wham", ParameterPack::KeyType::Required));
    whamPack_->ReadVectorString("outputs", ParameterPack::KeyType::Optional, VectorOutputNames_);
    whamPack_->ReadVectorString("outputFile", ParameterPack::KeyType::Optional,
                                VectorOutputFileNames_);
    ...
    ASSERT((VectorOutputNames_.size() == VectorOutputFileNames_.size()),
           "The output and the output files size is different.");
    registerOutput("histogram",
                   [this](std::string name) -> void { this->printTimeSeriesBins(name); });
    registerOutput("Autocorrelation",
                   [this](std::string name) -> void { this->printAutocorrelation(name); });
...
void Wham::isRegistered()
{
    for (int i = 0; i < VectorOutputNames_.size(); i++) {
        ...
        ASSERT((it != MapNameToFunction_.end()),
               "The output with name " << VectorOutputNames_[i] << " is not registered.");
    }
}

void Wham::initializeTimeSeries()
{
    ASSERT((VectorTimeSeries_.size() == 1 || VectorTimeSeries_.size() == Biases_.size()),
           "You can either provided a time series with \
    all the data combined or time series that are equal to size of biases while there are "
               << Biases_.size() << " biases but just \
    " << VectorTimeSeries_.size()
               << " time series.");
    ...
    for (int i = 0; i < VectorTimeSeries_.size(); i++) {
        ...
        if (verbose_) {
            std::cout << "Length of data for " << i << " is " << VectorTimeSeries_[i]->getSize()
                      << std::endl;
        }
    }
    ...
    if (VectorTimeSeries_.size() > 1) {
        ...
        for (int i = 0; i < dimensions_.size() - 1; i++) {
            ASSERT((dimensions_[i] == dimensions_[i + 1]),
                   "The dimension in the " << i << "th timeseries does not match with the " << i + 1
                                           << "th time series");
        }
        ...
        ASSERT((Ntot_ == xi_.size()),
               "The inputted Nvec or N does not sum up to the size of xi, one is "
                   << Ntot_ << " while latter is " << xi_.size());
    }
...
void Wham::initializeBias()
{
    ...
    // no longer needed
    // ASSERT((biases.size() == VectorTimeSeries_.size()), "The number of time series does not match
