...
SparseSampling::SparseSampling(const WhamInput& input) : Wham(input)
{
    initializeBias();

    for (int i = 0; i < VectorTimeSeries_.size(); i++) {
        ASSERT((VectorTimeSeries_[i]->getMean().size() == 1),
               "The dimension of the data must be 1.");
        means_.push_back(VectorTimeSeries_[i]->getMean());
        ASSERT((VectorTimeSeries_[i]->getstd().size() == 1),
               "The dimension of the data must be 1.");
...
void SparseSampling::calculate()
{
    ...
    ASSERT((sparseIntegration_.size() == VectorTimeSeries_.size()),
           "The sparse sampling integration result size does not match with \
...
void SparseSampling::printFE(std::string name)
{
    ...
    for (int i = 0; i < VectorTimeSeries_.size(); i++) {
        ofs_ << means_[i][0] << "\t" << preFactor_[i] << "\t" << energy_[i] << "\t"
             << sparseIntegration_[i] << "\t" << FE_[i] << "\n";
    }
    ofs_.close();
}
