...
public:
    ...
    virtual ~Uwham() {};
    void initializeStrat(Matrix<Real>& BUki, std::vector<Real>& N,
                         std::vector<stratptr>& strategies);
    void initializeBUki();
    void bindata(std::vector<std::vector<Real>>& x,
                 std::map<std::vector<int>, std::vector<int>>& map,
                 std::vector<std::vector<int>>& DataBinIndex);

    // This needs to be called after calculation of BUki --> calculates the initial guess using BAR
    // method
    void MakeInitialGuess(const Matrix<Real>& BUki, const std::vector<Real>& N,
                          std::vector<Real>& fk);
    ...
    void calculateFreeEnergy(const std::vector<Real>& lnwji,
    ...
    const std::map<std::vector<int>, std::vector<int>>& getMapBinIndexTolnwjiIndex()
    {
        return MapBinIndexTolnwjiIndex_;
    }
