...
SquaredBias::SquaredBias(const ParameterPack& pack) : Bias(pack)
{
    // read in dimension
    ...
    ASSERT((static_cast<int>(phi_.size()) == dimension_),
           "The size of the phi input does not match dimension.");
}

SquaredBias::Real SquaredBias::calculate(const std::vector<Real>& x)
{
    int size = x.size();
    ASSERT((size >= dimension_),
           "The dimension of the bias=" << xstar_.size() << " must be smaller than \
...
std::vector<SquaredBias::Real> SquaredBias::calculateForce(const std::vector<Real>& x)
{
    int size = x.size();
    ASSERT((size >= dimension_),
           "The dimension of the bias=" << xstar_.size() << " must be smaller not \
    the input data size = " << x.size());
