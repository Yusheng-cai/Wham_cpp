...
void appendGridPoint(const std::vector<const Bin*>& bins, int dataDimension,
                     const BinSelection& selection, BwhamBinning::BinGrid& grid)
{
    ...
    for (std::size_t i = 0; i < bins.size(); i++) {
        ...
        ASSERT((dimension >= 0 && dimension < dataDimension),
               "BWHAM bin dimension is outside the data dimension.");
    ...
    auto inserted =
        grid.indexToFlat.insert(std::make_pair(binIndex, static_cast<int>(grid.centers.size())));
    ASSERT((inserted.second), "There is duplicate BWHAM bin index.");
    grid.centers.push_back(center);
}

void buildGridRecursive(const std::vector<const Bin*>& bins, int dataDimension,
                        std::size_t binNumber, BinSelection& selection, BwhamBinning::BinGrid& grid)
{
...
BwhamBinning::BinGrid BwhamBinning::buildBinGrid(const std::vector<const Bin*>& bins,
                                                 int dataDimension)
{
    ASSERT((!bins.empty()), "BWHAM requires at least one bin definition.");
    ASSERT((static_cast<int>(bins.size()) == dataDimension),
           "The number of BWHAM bin definitions must match the data dimension.");
...
bool BwhamBinning::findBinIndexForSample(const std::vector<const Bin*>& bins,
                                         const std::vector<Real>& sample,
                                         std::vector<int>& binIndex)
{
    ASSERT((sample.size() == bins.size()),
           "BWHAM sample dimension must match the number of bin definitions.");

    binIndex.assign(bins.size(), 0);

    for (const Bin* bin : bins) {
        int dimension = bin->getDimension() - 1;
        ASSERT((dimension >= 0 && dimension < static_cast<int>(sample.size())),
               "BWHAM bin dimension is outside the sample dimension.");
