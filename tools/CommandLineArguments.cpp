...
CommandLineArguments::CommandLineArguments(int argc, char** argv) : argc_(argc)
{
    argv_.clear();
...
void CommandLineArguments::ParseArgv()
{
    ...
    for (int i = 0; i < argv_.size(); i++) {
        ...
        // First check if the argv is empty, then check if the first char is "-", then check if it
        // is a number
    ...
    for (int i = 0; i < num_keys_; i++) {
        ...
        // begin is the next charater after the one with "-", end is either end of string or the
        // next "-"
        int begin = key_idx + 1;
        int end;
