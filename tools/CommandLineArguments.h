...
public:
    ...
    template <typename T> bool readValue(const std::string& key, const Keys, T& value) const;
...
template <typename T, std::size_t dim>
bool CommandLineArguments::readArray(const std::string& key, const Keys type,
                                     std::array<T, dim>& arr) const
{
    ...
    ASSERT((temp_vec.size() == dim), "The size of the array passed in is "
                                         << temp_vec.size() << ". However the required amount is "
                                         << dim);
...
template <typename T>
bool CommandLineArguments::readVector(const std::string& key, const Keys type,
