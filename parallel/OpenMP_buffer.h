...
namespace OpenMP {
template <typename T> class OpenMP_buffer
{
public:
    ...
    OpenMP_buffer(T& master_obj) : master_object_ptr_(&master_obj)
    {
        buffer_.resize(OpenMP::get_max_threads());
    }
    ...
    T& access_buffer_by_id()
    {
        ...
        if (id == 0 && master_object_ptr_ != nullptr) {
            ...
            // pointer has its own memory address but reference shares the mem address with the
            // original obj
