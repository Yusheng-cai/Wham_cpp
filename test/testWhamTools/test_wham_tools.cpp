...
int main()
{
    ...
    failures += require_near("EXP", WhamTools::EXP(std::vector<double>{0.0, 0.0}), 0.0, 1e-12);

    failures += require_near(
        "CalculateDeltaFBarIterative",
        WhamTools::CalculateDeltaFBarIterative(std::vector<double>{0.2, 0.4, 0.6},
                                               std::vector<double>{-0.1, 0.1, 0.3}, 500, 1e-12),
        0.15, 1e-8);

    failures += require_near("CalculateDeltaFBarBisection",
                             WhamTools::CalculateDeltaFBarBisection(
                                 std::vector<double>{0.0, 0.0}, std::vector<double>{0.0, 0.0}, 500),
                             0.0, 1e-8);

    failures += require_near(
        "CalculateDeltaFBarBisection nonzero",
        WhamTools::CalculateDeltaFBarBisection(std::vector<double>{0.2, 0.4, 0.6},
                                               std::vector<double>{-0.1, 0.1, 0.3}, 500),
        0.15, 1e-6);
